...
# def create_pdf(image_list, output_pdf):
#     """Create a PDF from a list of image files."""
#     if not image_list:
#         logging.warning(f"No images found for PDF creation.")
#         return

#     try:
#         first_image = Image.open(image_list[0]).convert('RGB')  # Convert to RGB
#         first_image.save(output_pdf, save_all=True, append_images=[
#             Image.open(img).convert('RGB') for img in image_list[1:]
#         ])
#         logging.info(f"PDF created successfully: {output_pdf}")
#     except Exception as e:
...
#     # Walk through the base folder and process each subfolder
#     for root, dirs, _ in os.walk(base_folder_path):
#         for dir_name in dirs:
#             dir_path = os.path.join(root, dir_name)
#             image_files = find_images(dir_path)

#             # Create PDF using the folder name
#             if image_files:
#                 output_pdf_path = os.path.join(output_folder, f"{dir_name}.pdf")
#                 create_pdf(image_files, output_pdf_path)
#             else:
#                 logging.warning(f"No images found in folder: {dir_path}")
